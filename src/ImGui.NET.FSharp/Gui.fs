...
module Gui = 
    ...
    let SliderInt3 label (v: int ref) vMin vMax onChange = fun () ->
        if (ImGui.SliderInt3(label, &v.contents, vMin, vMax)) then
            onChange v.Value

    let SliderInt3_1 label (v: int ref) vMin vMax format onChange = fun () ->
        if (ImGui.SliderInt3(label, &v.contents, vMin, vMax, format)) then
            onChange v.Value

    let SliderInt3_2 label (v: int ref) vMin vMax format flags onChange = fun () ->
    ...
    let VSliderInt label (v: int ref) size vMin vMax onChange = fun () ->
        if (ImGui.VSliderInt(label, size, &v.contents, vMin, vMax)) then
            onChange v.Value

    let VSliderInt1 label (v: int ref) size vMin vMax format onChange = fun () ->
        if (ImGui.VSliderInt(label, size, &v.contents, vMin, vMax, format)) then
            onChange v.Value

    let VSliderInt2 label (v: int ref) size vMin vMax format flags onChange = fun () ->
